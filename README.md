...
内容包列表自动缓存：软件崩溃、意外关闭或正常退出后再打开，会询问是否恢复上次的列表（含译文与进度），选择不恢复则清除缓存：
...
- **游戏目录模式交互优化**：版本行旁新增勾选按钮（只管该版本）；版本详情页模组/资源包/光影包分类可折叠展开，分类旁 Checkbox 全选该分类内容包；选择面板版本分组可折叠展开，旁 Checkbox 全选该分组；全选/全不选合并为单个切换按钮（面板内同理）
- **性能优化**：折叠状态下不渲染子项，减少 DOM 节点，内容包数量大时切换流畅不卡顿
