...
- **风险提示**：免费模型通常有账号级速率限制，多线程容易触发 429，程序会自动退避重试。具体线程数以各服务商官网限速为准：OpenRouter 的免费模型可开 8 线程，智谱免费模型（glm-4-flash 系列）建议 1-2 线程，付费模型（glm-4.5 等）可开 4-8 线程提速明显。同模型批量翻译可保持译名风格一致。
...
- **导出失败兜底提示**：失败时展示目标路径与可能原因（磁盘空间不足 / 无写入权限 / 文件被其他程序占用）
- **解析失败分类提示**：区分「文件损坏或被占用」与「无法识别为可翻译内容」两类，分别给出处理建议
- **提示文案中英双语**：报错与操作提示接入界面语言切换，英文界面显示英文提示
- **生产构建内嵌前端资源**：安装包与免安装 exe 均内嵌界面资源，可独立运行，不再依赖本地开发服务器
