...
设置弹窗同样生效：服务商网格只留图标（品牌色本身已是强标识，名称交给悬停与读屏），窄窗口下不再被 11 家名称撑成三行：
...
- 主题 / 语言 / 无字模式从设置弹窗移到顶栏右上角（GitHub 按钮左侧），**点一下即改**：不弹确认、不弹成功提示（外观变化本身就是反馈），也不必先进设置再保存；主题与语言切换保留过渡动画
- 三个开关的图标都表达**当前状态**：亮色=太阳 / 暗色=月亮，语言显示当前语言码（ZH / EN）点开选择，无字模式开启时按钮转强调色描边 —— 状态一眼可见，不必点开才知道现在是哪一档
- 「个性化设置」随之瘦身：只留导出命名偏好与关闭行为这两项「需要想一下、适合一并保存」的设置，灰字描述同步改写

**无字模式（只留图标）**

- **一个开关收起整个外壳**：顶栏 / 侧栏 / 主工具栏 / 页脚的文字原地收起、只留图标；译文表格、术语表、日志、内容包卡片等**内容区一字不动**（游戏目录页按明确要求单开一个作用域，只收按钮与分类行）
- **悬停名与无障碍名**：无字模式下所有图标按钮都有悬停名称 —— 含禁用态（禁用按钮不派发鼠标事件，得由一层未禁用元素承接悬停），读屏另有 `aria-label`；服务商网格、拖拽区、设置弹窗内的图标按钮、游戏目录分类行一并补齐，两个垃圾桶图标不再分不清
- **灰字精简**：按「重复信息 → 删 / 格式约定与操作顺序 → 挪到悬停 / 数据读数与不可逆告知 → 留」逐处判定（全应用 38 处：隐藏 14、改悬停 7、保留 17）。收起翻译加速的限流与包数警告、本地数据说明、关于页的软件简介与作者署名、深度扫描的模板约定、游戏目录的选目录引导与空态说明；「扫描于 xx:xx」这类读数、删除类不可逆告知、空态交互引导一律保留
- **按钮与图标同步放大**：放大按钮时图标同步放大（常规 18px、主入口 20px）；「打开游戏目录」保持与常规档一致的占位（96px），不再缩成小方块；同排的纯图标按钮（重新扫描）一并抬到同一档，整行不失重；游戏目录 5 个分类按钮由 26 提到 34×28
- **按钮自绘图标**：不再复用「向内挤压」语义的现成图标，改为自绘的「窗口 + 左侧窄条」图形，直接表达"外壳收窄、内容不动"
- **文案与形态分档**：服务商网格在无字模式下只留图标（品牌色本身已是强标识，名称改由悬停与读屏承担），拖拽区两段说明收起、改为悬停中央大图标查看 —— 常规档逐字保留原实现，**不开无字模式即零视觉差异**

**游戏目录：扫描缓存与重新扫描（新）**

- **扫描缓存**：扫描结果缓存到软件数据目录（内存 LRU 8 条 + 落盘 `scan-cache-gamedir.json`），**重启后不必重扫**同一目录；路径按「小写 + 去尾部分隔符」归一，刻意不设过期时间 —— 缓存的意义就是"别再扫一遍"
- **重新扫描**：左栏新增按钮，忽略缓存强刷当前目录；重扫失败或被取消时保留旧结果与旧缓存，右侧不会闪成空态
- **命中缓存不再弹「扫描完成」汇总**：那是一份一次性报告，切目录时反复弹出反而碍事；结果的时间与来源改由左栏常驻的「扫描于 14:52」+「缓存」标签承担（跨天显示为「9-15 14:52」，否则三天前的结果和五分钟前的长得一模一样）
- **清理口径**：扫描缓存按「能否由软件重新生成、删掉不丢用户数据」归入缓存组，「清除缓存」同时清掉内存与磁盘两层

**界面与动效**

- **模式切换双向都有动效**：此前只有「自由导入 → 游戏目录」有入场动画（靠重挂载），反向没有（游戏目录面板常驻挂载、只用 display 切换）；现在动效挂在激活态类名上，两个方向一致
- **删除列表项改为「横向滑出 + 高度收起」**：行向右滑出淡出、外层同时把高度匀速收到 0，下方行是被"推"上来的，全程连续无瞬跳（术语表、深度扫描自定义规则、最近游戏目录三处统一）
- **切页与切服务商错峰滑入**：深度扫描三个页签、服务商切换的整块内容按 40ms 递进滑入，手感与开发者工具面板一致
- **游戏目录版本列表接入共享元素滑动指示器**：与左侧「内容包类型」导航同一套手感，消掉原先选中项的背景硬切
- **设置弹窗**：左侧分组由 148px 加到 200px 的加大档，底色与页面同色（两栏分界交给已有的分隔线）；弹窗宽度 760 → 880；API Key 与模型改用 `currentColor` 自绘图标 + 悬停说明；「清除缓存 / 清除用户数据」补图标与悬停名
- **服务商网格常规档改纯图标**：11 家服务商改为纯图标网格，每格带悬停名称与读屏名，不再把名称写两遍
- **「开始 AI 翻译」加宽**：改为占位而非尺寸（`min-width: 116px`）—— 加宽只影响自己，抬高会连带整行；116 是跨语言护栏，切到英文也不会缩水
- **图标同源**：内容包类型图标收敛为一份定义（此前散在四处各写一遍），侧栏导航、卡片标签、游戏目录分类行、筛选按钮共用，同一个类型不再长成两副样子

**修复**

- **自定义规则重启后丢失**：根因不在持久化本身，而在前端多处保存都写「内存里的整份设置」—— 深度扫描规则、AI 命名缓存、术语表、设置页表单各存各的，任何一处用过期的快照写回，就会覆盖别处刚写入的内容（导入的自定义规则正是被这样抹掉的）。新增 `patch_settings` 按需增量保存（对象逐键合并、数组整体替换、`null` 表示删除），所有保存点各写各的那部分
- **顺带修掉的隐患**：`settings.json` 解析失败时原先静默退回默认值，之后任何一次保存都会把用户的 API Key / 规则 / 术语表**永久覆盖** —— 现在先把原文件改名留档，再退回默认
- **打开「插件」规则弹窗时装的是模组规则集**：弹窗是常挂载的、关闭状态下父组件按默认值传的是模组规则，而初始化快照只在关闭时刷新；首次打开插件编辑器时快照还停在模组那套 → 导入的插件规则一条都看不见，同时把模组的内置规则值装进了插件编辑器（插件默认开、模组默认关，肉眼难察觉）
- **自定义规则的勾选与内置开关是两份数据**：勾选单独存在 `customEnabled` 里，规则测试台拿不到它，必须保存后重开弹窗才生效；改为直接写进 `rules.custom[].enabled` 与内置开关完全同源，改完 400ms 内自动重跑
- **单包规则弹窗**：文案还指向已改组的「个性化设置」（改为「设置 → 深度扫描」）；隐藏不该出现在单包场景的「导入规则档案」，自定义规则行只保留勾选（新增/编辑仍只在设置里）；没有可勾选规则时给出明确指引而不是诱导新增
- **小窗口下「导出命名偏好」按钮错位**：三个按钮靠负边距共享相邻边框，而等分列宽（实测 ≈230px）小于按钮自然宽（≈261px），第三颗被挤到第二行后既拿不到左边框、又拿到右圆角，看起来就是"缺一条边"。改为第一列按内容定宽（`max-content`）永不折行
- **设置侧栏底色比页面深**：改为透明，分界交给已有的分隔线

### v3.1.1（2026-09）

- **无字模式（只留图标）**：顶栏一个按钮把界面外壳的文字原地收起只留图标（顶栏 / 侧栏 / 主工具栏 / 页脚），译文表格、术语表、卡片等内容区一字不动；图标按钮同步放大（一般图标 14→18px，主入口图标 20px），全部图标按钮带悬停名称（含禁用态）与读屏名，全应用 38 处说明性灰字按「重复信息删 / 格式约定挪到悬停 / 数据读数与不可逆告知留」三档精简
- **顶栏即时开关**：主题、界面语言、无字模式移到右上角，点一下即改，不弹确认也不弹保存提示；图标表达当前状态（太阳 / 月亮、语言码、无字模式强调色描边），「个性化设置」随之只留导出命名偏好与关闭行为
- **游戏目录扫描缓存**：扫描结果按目录缓存（内存 8 条 + 落盘），切回最近目录与重启软件都不必重扫；左栏常驻「扫描于 14:52」「缓存」标签，跨天显示日期；新增「重新扫描」忽略缓存强刷，失败或被取消时保留旧结果；命中缓存不再弹一次性汇总；缓存归入「清除缓存」的范围（内存 + 磁盘两层）
- **动效**：模式切换两个方向都有动效；删除列表项改为「横向滑出 + 高度收起」，下方行被连续推上来（术语表 / 自定义规则 / 最近目录三处统一）；深度扫描页签与服务商切换错峰滑入；游戏目录版本列表接入共享元素滑动指示器
- **界面**：服务商网格改纯图标（名称交给悬停与读屏），图标同源收敛为一份定义（内容包类型不再长成两副样子）；「开始 AI 翻译」按内容定宽（跨语言不缩水）；设置弹窗分组栏加宽、底色与页面同色；API Key / 模型 / 清除类按钮补自绘图标与悬停说明
- **修复**：导入的自定义规则保存后重启丢失（多处保存各写整份内存设置 → 改为 `patch_settings` 增量保存）；`settings.json` 解析失败会静默清空用户数据（改为先留档 `settings.corrupt-*.json`）；打开插件规则弹窗装载的是模组规则集；自定义规则勾选与规则测试台不同步；小窗口下导出命名偏好按钮错位；单包规则弹窗文案指向已改组的设置页
