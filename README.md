...
| 🔍 **模组深度扫描** | 普通解析不到文本时自动/手动扫描模组内全部文本（成就/配置/嵌套 jar 等），分组勾选、默认不勾选防误翻；导出含内嵌文本时强风险提示 |
| 🧠 **自定义提示词** | 模组 / 光影包 / 资源包三套提示词分开定制：角色/语境/参考术语可改，核心规则（占位符/术语表/JSON 输出）系统锁定不可破坏，支持「恢复默认」 |
| ✨ **§ 格式码预览** | 译文含 § 颜色/样式码时，悬停即可预览最终游戏内效果（颜色/粗体/斜体/下划线） |
| 📂 **导出后打开文件夹** | 导出成功右下角通知 + 一键「打开所在文件夹」 |
| 🔄 **版本检查** | 启动静默检查更新（访问不了 GitHub 无感）+ 设置里手动「检查更新」一键跳转 Release |
| ⏱ **剩余时间估算** | 进度条旁按当前速率实时推算预计剩余时间 |
| 📖 **术语管理闭环** | 自动提取的术语可一键加入用户术语表（全局统一译名）；翻译完成后建议高频词入表 |
...
- **模组深度扫描**：类型探测支持数据包式模组（mod 元数据优先于 pack.mcmeta）；无语言文件的模组不再报错，引导启用深度扫描
- 深度扫描覆盖成就/配置/嵌套 jar 等内嵌文本，分组勾选 + 默认不勾选 + 导出风险提示

### v1.1.0（2026-08）

- **自定义提示词**：模组/光影包/资源包三套分开定制；可编辑段（角色/语境/参考）开放，核心段（占位符保护/术语表/JSON 输出）系统锁定 + 灰色只读 + 恢复默认
- **§ 格式码效果预览**：悬停译文预览最终颜色/样式
- **导出后「打开所在文件夹」**：右下角通知一键定位
- **版本检查**：启动静默检查 + 设置手动检查（连不上 GitHub 明确提示）
- **剩余时间估算**：进度条实时推算
- **术语管理闭环**：提取的术语一键加入用户术语表；翻译完成建议高频词入表
- **优化**：检查更新命令区分「最新/有新版/网络失败」三态
