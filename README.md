...
- **清理空间（推荐用界面按钮）**：设置 →「关于 → 本地数据」里可直接清理，分两档——
...
- **文本解析**：普通扫描覆盖 `plugin.yml` 的命令描述/权限描述/插件简介、`config.yml` 与 `lang/`、`locale(s)/`、`messages/` 下的 yml / properties / json 语言文件；自动过滤数据库、权限节点、冷却时间等**游戏内不可见的内部配置**；深度扫描额外提取 `.class` 常量池里的硬编码文本（聊天前缀、GUI 标题、计分板等）
