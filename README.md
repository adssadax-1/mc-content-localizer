...
| 多服务商 | 13 家预设：云端 11 家（智谱 / Gemini / DeepSeek / 通义 / 豆包 / Kimi / 混元 / 硅基流动 / OpenAI / OpenRouter / 自定义）+ **本地推理 2 家（Ollama / llama.cpp，免 API Key）**；Key 与模型独立保存，一键拉取模型列表、验证连接 |
...
- **只收外壳，不收内容**：服务商页的各家提示段（豆包 / Kimi / OpenAI / OpenRouter / Ollama / llama.cpp / 自定义）与本地档「地址可改端口或走局域网」的补充说明，在无字档整段收起，给图标网格让位；**常规档一字未动**
- **顶栏两条切换栏**（模式 / 内容包类型）在同排按钮统一尺寸，图标同步放大，三组控件等高对齐
...
- **记住上次停留的设置分组**：关闭设置再打开会回到上次那一页；只记住页面本身，不写入磁盘、不触发保存，软件退出即复位
...
在程序右上角「设置」中配置翻译服务，各服务商独立保存 Key 与模型选择。设置左侧分组：个性化设置 / 服务商 / 翻译参数 / 术语表 / **深度扫描** / 翻译加速 / 关于，深度扫描规则与「关于 → 本地数据」的清理入口都在里面（见下文对应章节）。
内置 13 家服务商预设：云端 11 家（智谱 / 通义 / DeepSeek / 火山豆包 / Kimi / 腾讯混元 / 硅基流动 / Google Gemini / OpenAI / OpenRouter / 自定义）+ 本地推理 2 家（Ollama / llama.cpp，免 API Key）。图标网格点选即切，各服务商 Key 与模型独立保存。模型列表是可搜索下拉、免费模型带「免费」标注并优先排序，预设服务商 BaseURL 只读展示并附官网跳转，「验证连接」可一键检测所选模型是否可用：

![模型服务商](docs/模型服务商预设扩展.png)
...
预设里带两家本地推理服务，**不用填 API Key**，模型名由你本地已拉取的模型决定：

| 服务 | 默认地址 | 说明 |
|---|---|---|
| Ollama | `http://localhost:11434/v1` | 模型名取自 `ollama list`；地址可改端口或走局域网 |
| llama.cpp | `http://127.0.0.1:8080/v1` | `llama-server` 默认免鉴权；模型名取自启动参数里的 `--alias` |

本地档做了几处针对性处理：

- **不发送 Authorization 头**：本地服务端本来就不校验；Key 留空即可保存，切回云端档时仍会要求填 Key
- **绕过系统代理**：机器上开着代理工具时，发往本机端口的请求不再被代理劫持（此前表现为本地模型莫名连不上）
- **超时放宽**：本地推理冷启动要加载权重、单并发还要排队，单请求超时比云端档宽松得多
- **响应解析更宽容**：不再要求模型返回"完美 JSON"——带代码块围栏、前后夹着解释文字、甚至被截断的输出都能尽量读出可用条目；部分小模型不回显条目 key、直接给原文也能对上；key 的大小写与空格差异不再导致漏条
- **批次自动补漏**：整批「有返回但不可用」时，只把未拿到的条目重发一次并追加更严格的格式要求（此前这类批次直接作废，一条都不留）
- **未翻译原因可辨**：备注列能区分「模型没有返回」与「返回了但内容不可用（具体原因）」，便于判断是网络/限流还是模型格式问题
...
- **新增本地模型（Ollama / llama.cpp）**：免 API Key、不发送 Authorization 头，默认地址 `localhost:11434` / `127.0.0.1:8080`；模型名取自本地已拉取的模型
- **本地推理绕过系统代理**：开着代理工具时，发往本机端口的请求不再被代理劫持（此前表现为本地模型连不上）
- **本地档容错增强**：单请求超时放宽；响应解析不再要求"完美 JSON"——带围栏、夹解释文字、被截断的输出都能尽量读出条目，key 的大小写与空格差异不再漏条
- **批次自动补漏**：整批「有返回但不可用」时只重发未拿到的条目并追加更严格的格式要求，不再整批作废
- **未翻译原因可辨**：备注列区分「模型没有返回」与「返回了但内容不可用（具体原因）」
- **无字模式补齐**：服务商页各家提示段与本地档补充说明在无字档整段收起（常规档不动）；顶栏两条切换栏与同排按钮统一尺寸、图标同步放大
- **设置弹窗记住上次停留的分组**（仅内存，退出即复位）
- **修复**：顶栏垂直对齐——左上图标与标题、右侧一组按钮此前按文字基线对齐而上下错开，现统一按中轴对齐（常规与无字两档都生效）
