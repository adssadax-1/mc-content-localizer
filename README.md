...
| 游戏目录模式 | 扫描 `.minecraft` / `versions` / 任意文件夹，按版本分组展示模组、服务器插件、资源包、光影包（`plugins/` 与 `mods/` 并存时分别归类，按内容判定而非文件夹名）；勾选一键入队，跨版本译文复用，导出按版本分组；上千个内容包批量导入不卡顿 |
...
设置 →「个性化设置」集中放界面与导出偏好：主题（明 / 暗）、界面语言（中 / 英）、**导出命名偏好**、关闭行为（直接退出 / 最小化到托盘）：
...
- 深度扫描开关已独立成「深度扫描」设置页，模组与插件分别配置，点开即见规则弹窗
